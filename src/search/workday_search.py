...
class WorkdaySearcher(BaseSearcher):
    ...
    def search(self, keywords: str, location: str = "remote", max_results: int = 10) -> list[JobListing]:
        ...
        time.sleep(4)
        jobs = []
        links = self.page.query_selector_all("a[data-automation-id='jobTitle']")[:max_results]
        for link in links:
            try:
                title = link.inner_text().strip()
                href = link.get_attribute("href") or ""
                url_full = href if href.startswith("http") else f"{self.company_url.rstrip('/')}{href}"
                jobs.append(JobListing(title=title, company="", url=url_full, source="workday"))
            except Exception:
                continue
        return jobs
    ...
    def scrape_description(self, job: JobListing) -> str:
        ...
        time.sleep(2)
        try:
            desc = self.page.query_selector("[data-automation-id='jobPostingDescription']")
